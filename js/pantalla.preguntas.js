class PantallaPreguntas{
    ...
    mostrarVista( pregunta_obtenida ){
        ...
        for (let c=0; c< this.ultima_pregunta.r.length; c++){
            ...
            this.textos_respuestas[c] = this.juego.add.text(this.juegoConf.width*0.01 +160, this.juegoConf.height/1.2 -(i)*160 -diffY, '', 
                            { fontFamily: 'Arial Rounded MT, Arial, sans-serif', fontSize: 40, color: '#000000' });
...
//Temporizador
iniciarTemporizador() 
    let tiempoRestante = 23; // Duración del temporizador en segundos
    const estiloTexto = {
        fontFamily: 'Arial',
        fontSize: 24,
        color: '#000000',
        ...
        boxShadow: '0 0 5px rgba(0, 0, 0, 0.3)' // Sombra
    };
    const tiempoMostrar = this.juego.add.text(this.juegoConf.width * 0.5, this.juegoConf.height * 0.05, '', estiloTexto);
    const intervalo = setInterval(() => {
        tiempoMostrar.setText(`Tiempo restante: ${tiempoRestante}s`);
        tiempoRestante--;
        if (tiempoRestante < 0) {
            clearInterval(intervalo);
            this.pasarSiguientePantalla();
        }
    }, 1000); // Actualizar cada segundo


// Método para pasar a la siguiente pantalla
pasarSiguientePantalla() 
    console.log('Se agotó el tiempo.');
    // Código para pasar a la siguiente pantalla aquí
